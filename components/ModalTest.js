...
function NewListModal() {
  ...
  const { openTestModal, setOpenTestModal } = useContext(Context);
  ...
  return (
    <Transition.Root show={openTestModal} as={Fragment}>
      <Dialog
        as="div"
        className="fixed z-50 inset-0 overscroll-y-auto "
        onClose={setOpenTestModal}
      >
        <div className="flex items-center justify-center min-h-[800px] sm:min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
          ...
          >
            <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-4  pb-4 text-left overflow-hidden shadow-xl transform  transition-all sm:my-8 sm:align-middle sm:max-w-sm sm:w-full">
              <div>
